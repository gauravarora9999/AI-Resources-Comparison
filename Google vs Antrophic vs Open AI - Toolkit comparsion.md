...
- **Subagent token sprawl.** Claude Code Pro/Max users have reported 3–50× faster rate-limit consumption after specific version regressions (Claude Code v2.1.89 in March 2026 was one such case). Build cost telemetry into your agent layer; don't rely solely on vendor dashboards.
 Project Mariner / WebVoyager 83.5% is now a *historical* number — Mariner was shut down May 4, 2026. Any "OpenAI Operator" reference should be replaced with "Atlas + ChatGPT Agent". DALL·E references should be replaced with ChatGPT Images 2.0 (gpt-image-2 / gpt-image-1 / gpt-image-1-mini). Any "Vertex AI" branding should be updated to "Gemini Enterprise Agent Platform" (the SKUs and APIs are unchanged). The "Claude Code SDK" reference should be updated to "Claude Agent SDK". Sora 2 references should note the September 24, 2026 API sunset.
